...
static void flush_full_items(encoder* self)
{
    ...
    self->writer(full_data->first_item, full_data->length * sizeof(uint64_t), self->writer_params);
    ...
    for (uint32_t i = 0; i < length; i++)
    {
        ...
        else
        {
            ...
            for (uint8_t bit = 0; bit < 8; bit++)
            {
                ...
                bit_buffer_add_bit(&self->output_buffer, current_bit);
            }
        }
...
void encoder_flush(encoder* self)
{
    ...
    if (bits_left)
    {
        ...
        {
            bit_buffer_add_bit(&tmp_buffer, (bytes_to_write[bytes_to_write_count - 1] >> i) & 1);
        }
        bit_buffer path_holder;
        bit_buffer_init(&path_holder);
        adaptive_node_get_path(self->tree.nyt_node, &path_holder);
